...
export default function MenuPage() {
  ...
  // setOutlets - populate outlets in useEffect
  useEffect(() => {
    const fetchMenu = async () => {
      const res = await fetch('http://localhost:5555/api/menu', {
        credentials: 'include'
      });
  ...
  const handleFavourite = async (itemID) => {
      ...
      prev.map(outlet => ({
        ...
        items: outlet.items.map(item =>
          ...
          ? {
              ...
              favourite_count: item.favourited
    ...
    try {
      const res = await toggleFavourite(itemID, token);

      // authoritative backend update
      setOutlets(prev =>
        prev.map(outlet => ({
          ...outlet,
          items: outlet.items.map(item =>
            item.id === itemID
              ? {
                  ...
                  favourite_count: res.favourite_count
                }
              : item
          )
        }))
                      ...
                      >
                        ...
                        <div className="p-4 space-y-3">
                          ...
                          <div className="flex items-center gap-2 pt-2">
                            ...
                            <button disabled = { !isLoggedIn } onClick={ () => handleFavourite(item.id) } className="p-2.5 bg-secondary rounded-xl transition-colors">
                              <Heart className={`w-5 h-5 transition-colors ${ item.favourited ? 'text-red-500 fill-red-500' : 'text-muted-foreground hover:text-red-500'}` } />
                            </button>
                          </div>
                        </div>
                      </div>
